...
- **Keep architecture facts separate from their representations.** The linked semantic graph is the authoritative model. C1, C2, C3, Deployment, source-focused, and custom views are derived from that graph rather than maintained as independent diagrams.
...
- **Derive views for the question at hand.** Graph queries can select architectural levels, source scopes, relationships, inherited types, derived dependencies, deployment projections, and visual groups. Built-in C1, C2, C3, Deployment, and unfiltered views are ordinary queries over the same linked model.
